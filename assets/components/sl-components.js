...
class JWAccordion extends HTMLElement {
    ...
    render() {
        const items = Array.from(this.querySelectorAll('jw-accordion-item, sl-accordion-item'));
...
class JWTabs extends HTMLElement {
    ...
    render() {
        const tabList = Array.from(this.querySelectorAll('jw-tab, sl-tab'));
...
class JWDragDropAlt extends HTMLElement {
    ...
    render() {
        const items = Array.from(this.querySelectorAll('jw-item, sl-item')).map(item => item.textContent);
...
class JWHotspotContainer extends HTMLElement {
    ...
    render() {
        ...
        const markers = Array.from(this.querySelectorAll('jw-hotspot-marker, sl-hotspot-marker'));
...
class JWTimeline extends HTMLElement {
    ...
    render() {
        const items = Array.from(this.querySelectorAll('jw-timeline-item, sl-timeline-item'));
        const idBase = Math.random().toString(36).substr(2, 9);
...
class JWMatchingGame extends HTMLElement {
    ...
    render() {
        const pairs = Array.from(this.querySelectorAll('jw-match-pair, sl-match-pair'));
...
class JWScenario extends HTMLElement {
    ...
    render() {
        ...
        const initialBranch = this.querySelector('jw-branch[initial], sl-branch[initial]') || this.querySelector('jw-branch, sl-branch');
        if (!initialBranch) return;

        // Hide all original templates
        this.querySelectorAll('jw-branch, sl-branch').forEach(b => b.style.display = 'none');
    ...
    showBranch(branchId) {
        ...
        const branchTemplate = Array.from(this.querySelectorAll('jw-branch, sl-branch')).find(b => b.id === branchId);
        if (!branchTemplate) return;

        const title = branchTemplate.getAttribute('title') || 'Scenario';
        const choices = Array.from(branchTemplate.querySelectorAll('jw-choice, sl-choice'));

        // Clean the content of choice tags to avoid double-text
        const contentClone = branchTemplate.cloneNode(true);
        contentClone.querySelectorAll('jw-choice, sl-choice').forEach(choice => choice.remove());
...
class JWWizard extends HTMLElement {
    ...
    render() {
        const steps = Array.from(this.querySelectorAll('jw-step, sl-step'));
...
class JWInteractiveTable extends HTMLElement {
    ...
    render() {
        const headers = Array.from(this.querySelectorAll('jw-header, sl-header')).map(h => ({
            text: h.textContent,
            type: h.getAttribute('type') || 'string'
        }));
        const rows = Array.from(this.querySelectorAll('jw-row, sl-row')).map(row => 
            Array.from(row.querySelectorAll('jw-cell, sl-cell')).map(cell => cell.innerHTML)
        );
...
class JWCarousel extends HTMLElement {
    ...
    render() {
        const slides = Array.from(this.querySelectorAll('jw-slide, sl-slide'));
...
class JWMultiColumn extends HTMLElement {
    ...
    render() {
        const columns = Array.from(this.querySelectorAll('jw-column, sl-column'));
        this.innerHTML = `
...
// Helper to register custom components under both sl- and jw- prefixes for backwards compatibility
function registerComponent(slName, jwName, elementClass) {
    if (!customElements.get(slName)) {
        customElements.define(slName, elementClass);
    }
    if (!customElements.get(jwName)) {
        // Use an empty subclass to avoid DOMException constructor reuse constraints
        customElements.define(jwName, class extends elementClass {});
    }
}

// Register Components (support both sl- and jw- prefixes)
registerComponent('sl-accordion', 'jw-accordion', JWAccordion);
registerComponent('sl-tabs', 'jw-tabs', JWTabs);
registerComponent('sl-flip-card', 'jw-flip-card', JWFlipCard);
if (!customElements.get('sl-flipcard')) {
    customElements.define('sl-flipcard', class extends JWFlipCard {});
}
if (!customElements.get('jw-flipcard')) {
    customElements.define('jw-flipcard', class extends JWFlipCard {});
}
registerComponent('sl-dragdrop-alt', 'jw-dragdrop-alt', JWDragDropAlt);
registerComponent('sl-matching-game', 'jw-matching-game', JWMatchingGame);
registerComponent('sl-click-reveal', 'jw-click-reveal', JWClickReveal);
registerComponent('sl-hotspot-container', 'jw-hotspot-container', JWHotspotContainer);
registerComponent('sl-timeline', 'jw-timeline', JWTimeline);
registerComponent('sl-scenario', 'jw-scenario', JWScenario);
registerComponent('sl-wizard', 'jw-wizard', JWWizard);
registerComponent('sl-notifications', 'jw-notifications', JWNotifications);
registerComponent('sl-interactive-table', 'jw-interactive-table', JWInteractiveTable);
registerComponent('sl-carousel', 'jw-carousel', JWCarousel);
registerComponent('sl-modal', 'jw-modal', JWModal);
registerComponent('sl-form-validation', 'jw-form-validation', JWFormValidation);
registerComponent('sl-tooltip', 'jw-tooltip', JWTooltip);
registerComponent('sl-button-demo', 'jw-button-demo', JWButtonDemo);
registerComponent('sl-progress-bar', 'jw-progress-bar', JWProgressBar);
registerComponent('sl-multi-column', 'jw-multi-column', JWMultiColumn);
...
registerComponent('sl-link-demo', 'jw-link-demo', JWLinkDemo);
registerComponent('sl-quiz', 'jw-quiz', JWQuiz);
